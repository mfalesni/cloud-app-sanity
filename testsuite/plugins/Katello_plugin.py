...
class KatelloPlugin(object):
    @classmethod
    def poll_task_state(cls, server, port, task_uuid, login, password):
        ...
        :rtype: ``str``
        """
        request = Test.Network.make_auth_request("https://%s:%s/katello/api/systems/tasks/%s" % (server, port, task_uuid), login, password)
        response = urlopen(request)
        data = json.loads("\n".join(response.readlines()))
        return str(data["state"])

    @classmethod
    def system_group_create(cls, server, port, org, login, password, group_name):
        ...
        request = Test.Network.make_auth_request("https://%s:%s/katello/api/organizations/%s/system_groups" % (server, port, org), login, password)
        ...
        try:
            ...
            if int(e.code) in [202]:
                ...
                Test.Fail(msg="Error when creating system_group %s with HTTP code %d: %s" % (group_name, int(e.code), e))

        return json.loads(response.read())

    @classmethod
    def system_group_add_system(cls, server, port, org, uuid, login, password, group_id):
        ...
        # Prepare the request
        # POST /api/organizations/:organization_id/system_group
        request = Test.Network.make_auth_request("https://%s:%s/katello/api/organizations/%s/system_groups/%s/add_systems" \
        ...
        try:
            ...
            if int(e.code) in [202]:
                ...
                Test.Fail(msg="Error when adding system '%s' to group id '%s' with HTTP code %d: %s" % (uuid, group_id, int(e.code), e))

        return json.loads(response.read())

    @classmethod
    def system_group_query(cls, server, port, org, login, password, query_name=None):
        # Prepare the request
        ...
        request = Test.Network.make_auth_request(query_url, login, password)
        ...
        try:
            ...
            if int(e.code) in [202]:
                ...
                Test.Fail(msg="Error when querying system groups with HTTP code %d: %s" % (int(e.code), e))
        return json.loads("\n".join(response.readlines()))

    @classmethod
    def query_remote_install(cls, server, port, uuid, login, password, package):
        ...
        """
        # Prepare the request
        request = Test.Network.make_auth_request("https://%s:%s/katello/api/systems/%s/packages" % (server, port, uuid), login, password)
        request.add_header("content-type", "application/json")
        ...
        try:
            ...
            if int(e.code) in [202]:
                ...
                Test.Fail(msg="Error when querying installation of package %s with HTTP code %d: %s" % (package, int(e.code), e))
        # get the task uuid
        ...
        while state != "finished":
            ...
            if state not in ok_states:
                Test.Fail(msg="Installation of package %s failed when task went to state '%s'" % (str(package), state))
        # Package is installed, let's verify it
        assert Test.RPM.query(package)

export = KatelloPlugin
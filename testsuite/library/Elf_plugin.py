...
class ElfPlugin(object):
    ...
    def is_elf(filename):
        ...
        """
        try:
            assert Test.Run.command("readelf -h %s" % filename)
            return True
    ...
    def readelf(filename):
        ...
        :rtype: ``str``
        """
        return Test.Run.command("readelf --all %s" % filename).stdout

    @classmethod
    def fortify_find_dangerous(cls, filename):
        ...
        dangerous_calls = re.findall("(?:\\b|__)(?:" + "|".join(dangerous) + ")(?:\\b|_chk|__chk_fail)", cls.readelf(filename))
